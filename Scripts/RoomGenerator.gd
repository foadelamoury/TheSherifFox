...
func generate_dungeon(grid: Array[Array], branch_length: int, dead_end_chance: float, dead_end_max: int) -> Array[Array]:
	...
	_grid[current_y][current_x] = Room.new(Room.room_type.START, [Room.direction.NORTH, Room.direction.SOUTH, Room.direction.EAST])
	...
	for i in range(branch_length):
		...
		if directions:
			...
			# TODO: Add different room types
			var dx = current_coordinate[1] - last_coordinate[1]
			var dy = current_coordinate[0] - last_coordinate[0]
			...
			_grid[last_coordinate[1]][last_coordinate[0]].directions.append(room_dir)
			...
			push_error("GEN400 | Room has no valid directions.")
